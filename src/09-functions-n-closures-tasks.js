...
function getPolynom(...args) {
  return (x) => args.reduce((acc, item, indx, arr) => {
    const exp = arr.length - indx - 1;
    return acc + item * x ** exp;
  }, 0);
}
...
function logger(func, logFunc) {
  return ((...args) => {
    const argums = args.map((arg) => JSON.stringify(arg));
    const text = `${func.name}(${argums})`;
